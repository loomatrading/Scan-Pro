...
def detect_document_corners(image):
    ...
    edges = cv2.Canny(gray_blur, 35, 130)
...
def ai_super_resolution(img):
    """
    للمستندات النصية نحافظ على التفاصيل الأصلية.
    لا نستخدم EDSR لأنه قد يجعل الحروف غير طبيعية أو أكثر سماكة.
    """
...
def svg_icon(kind, color="#111111"):
    icons = {
        "original": (
            f'<rect x="10" y="6" width="28" height="36" rx="3" '
            f'fill="{color}" opacity=".12"/>'
            f'<rect x="10" y="6" width="28" height="36" rx="3" '
            f'fill="none" stroke="{color}" stroke-width="2.5"/>'
            f'<path d="M16 16h16M16 23h16M16 30h12M16 37h8" '
            f'stroke="{color}" stroke-width="2.5"/>'
        ),
        "ai": (
            '<text x="4" y="36" font-family="Arial" font-size="30" '
            'font-weight="700" fill="#00B89C">AI</text>'
            '<path d="M39 7l2 6 6 2-6 2-2 6-2-6-6-2 6-2z" '
            'fill="#18C9A7"/>'
        ),
        "trash": (
            f'<path d="M12 14h24M18 14V10h12v4'
            f'M15 14v22a2 2 0 002 2h14a2 2 0 002-2V14" '
            f'fill="none" stroke="{color}" stroke-width="3" '
            f'stroke-linecap="round" stroke-linejoin="round"/>'
            f'<path d="M20 20v10M28 20v10" stroke="{color}" '
            f'stroke-width="3" stroke-linecap="round"/>'
        ),
        "cleaner": (
            f'<path d="M15 34l18-18" stroke="{color}" '
            f'stroke-width="4" stroke-linecap="round"/>'
            f'<path d="M12 37l6-2-4-4z" fill="{color}"/>'
            f'<path d="M29 9l2 5 5 2-5 2-2 5-2-5-5-2 5-2z" '
            f'fill="{color}"/>'
        ),
        "rotate_single": (
            '<path d="M15 16a14 14 0 112 16" fill="none" '
            'stroke="#7B42F6" stroke-width="3.5" stroke-linecap="round"/>'
            '<path d="M15 9v8h8" fill="none" stroke="#7B42F6" '
...
def make_icon(kind, color="#111111", size=46):
    ...
    renderer = QSvgRenderer(
        svg_icon(kind, color).encode("utf-8")
    )
...
class InteractivePreview(QLabel):
    def __init__(self, parent=None):
        ...
        self.setStyleSheet(
            "background:#FFFFFF; color:#222; border-radius:4px;"
        )
        self.setSizePolicy(
        ...
        if event.modifiers() & Qt.ControlModifier:
            ...
            if delta > 0:
                self.zoom_factor = min(
                    self.zoom_factor * 1.15, 5.0
                )
            else:
                self.zoom_factor = max(
                    self.zoom_factor / 1.15, 0.5
                )
    ...
    def paintEvent(self, event):
        ...
        if (
            ...
            box_x = (
                self.width() - int(bw * b_scale)
            ) // 2

            box_y = (
                self.height() - int(bh * b_scale)
            ) // 2
    ...
    def erase_at(self, pos):
        ...
        if 0 <= img_x < w and 0 <= img_y < h:
            ...
            cv2.rectangle(
                self.image,
                (
                    img_x - self.brush_size,
                    img_y - self.brush_size
                ),
                (
                    img_x + self.brush_size,
                    img_y + self.brush_size
    ...
    def mousePressEvent(self, event):
        if (
            self.image is None
            and self.click_to_open_callback
        ):
        ...
        for i, h_pt in enumerate(self.get_all_handles()):
    ...
    def mouseMoveEvent(self, event):
        ...
        if (
            ...
            x = max(
                0,
                min(
                    (self.mouse_pos.x() - box_x) / b_scale,
                    bw
                )
            )
            ...
            y = max(
                0,
                min(
                    (self.mouse_pos.y() - box_y) / b_scale,
                    bh
                )
            )
...
class ScanPro(QMainWindow):
    ...
    def build_ui(self):
        ...
        QPushButton#tool[selected='true'] {
            background:#D9F7F1;
            border:1px solid #10B99A;
        }
        ...
        self.pages.currentRowChanged.connect(
            self.select_page
        )
        ...
        self.preview.corner_changed_callback = (
            self.on_corners_updated
        )
        self.preview.image_edited_callback = (
            self.on_image_edited
        )
        self.preview.click_to_open_callback = (
            self.open_image
        )
        ...
        self.add_overlay = ClickableOverlayFrame(
            self.open_image,
            self.preview
        )

        self.add_overlay.setCursor(
        ...
        self.add_overlay.setStyleSheet(
            "background:transparent;"
        )
        ...
        self.original_btn = self.tool_button(
            "original",
            "Original",
            "#1677FF",
        ...
        self.magic_btn = self.tool_button(
            "ai",
            "Magic Pro AI",
            "#00B89C",
            self.run_magic
        ...
        self.cleaner_btn = self.tool_button_custom(
            "cleaner.png",
            "Cleaner",
            "#0088FF",
            self.toggle_cleaner
        ...
        self.delete_btn = self.tool_button(
            "trash",
            "",
            "#D32F2F",
            self.delete_image,
    ...
    def tool_button(
        ...
        b = QPushButton()
        b.setObjectName(
            "delete" if is_delete else "tool"
        )
        b.setIcon(
            make_icon(kind, color, 46)
        )
    ...
    def update_overlay(self):
        ...
        if self.original is None:
            ...
            x = max(
                0,
                (self.preview.width() - w) // 2
            )
            y = max(
                0,
                (self.preview.height() - h) // 2
            )

            self.add_overlay.setGeometry(
                x, y, w, h
            )
    ...
    def update_button_styles(self):
        for btn in (
            self.original_btn.button,
            self.magic_btn.button,
            self.cleaner_btn.button
        ):
